...
export function simulate(config) {
  ...
  let currentGearIndex = (config.launchGear || 1) - 1; // 0-indexed (0 = 1. vites)
  ...
  let shiftTimeSec = shiftTimeMs / 1000;
  let targetLaunchRpm = config.launchRpm || (idleRpm + 2000);

  // Şoför Stili Modifikasyonları
  if (driverStyle === 'aggressive') {
    shiftTimeSec = Math.max(0.05, shiftTimeSec - 0.05); // 50ms daha hızlı vites
    targetLaunchRpm += 500; // Kalkışta daha fazla devir (patinaja zorlar)
  } else if (driverStyle === 'smooth') {
    shiftTimeSec += 0.15; // 150ms daha yavaş vites (risksiz)
    targetLaunchRpm -= 500; // Daha sakin kalkış (patinajı önler)
  }

  // Initial step setup
  let engineRpm = targetLaunchRpm;
  ...
  while (!isFinished && time_s < maxSimulationTime) {
    ...
    if (isShifting) {
      ...
      if (calculatedRpm < targetLaunchRpm && currentGearIndex === (config.launchGear || 1) - 1) {
        // Kalkış vitesinde ve hız düşükken debriyaj kaydırılıyor
      ...
      // BOOST BY GEAR (Vitese Göre Güç Sınırı)
      const gearNum = currentGearIndex + 1;
      if (config.boostByGear && config.boostByGear[gearNum]) {
        const hpLimit = config.boostByGear[gearNum];
        const currentHp = (engineTorque * Math.max(engineRpm, 1000)) / 7120.9;
        if (currentHp > hpLimit) {
          engineTorque = (hpLimit * 7120.9) / Math.max(engineRpm, 1000);
        }
      }
      ...
      if (grossWheelForce > maxTractionForce) {
        if (driverStyle === 'smooth') {
          // Pürüzsüz şoför gazı kısarak tam sınırda tutar (Traction Control gibi).
          // Dinamik sürtünme kaybı yaşamaz ama gaz kestiği için appliedForce sınırlanır.
          appliedForce = maxTractionForce;
          slipPct = 1; // Çok hafif kayma
          slipTimeTotal += dt * 0.1; 
        } else if (driverStyle === 'aggressive') {
          // Agresif şoför gaza oturur. Patinajda dinamik sürtünme statik sürtünmeden düşüktür (%15 kayıp).
          appliedForce = maxTractionForce * 0.85; 
          slipPct = ((grossWheelForce - maxTractionForce) / grossWheelForce) * 100;
          slipTimeTotal += dt;
        } else {
          // Balanced şoför
          appliedForce = maxTractionForce * 0.95; // %5 dinamik sürtünme kaybı
          slipPct = ((grossWheelForce - maxTractionForce) / grossWheelForce) * 100;
          slipTimeTotal += dt;
        }
        
        if (timeSeries.length % 50 === 0 && slipPct > 1) {
